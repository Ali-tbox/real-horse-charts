...
function StridePrecision({chartData, handleItemClick }) {
  ...
  const objWithPoorConfidence = chartData?.confidence?.filter(item => item.confidence === 'poor' && item.trottype !== 'allfootage')
  return (
    <Box w='100%' mt='32px' display={'flex'} flexDir={'column'} justifyContent={'center'} paddingX={'16px'}>
      ...
      {objWithPoorConfidence?.length > 0 && (
        <Box mb='32px' borderRadius='8px' paddingY='12px' paddingX={'16px'} bg={colors.faint} w='100%'>
          <Text fontFamily='Nunito' fontSize='14px' color={colors.textcolor} lineHeight={'20px'}>
            {`We recommend you repeat ${objWithPoorConfidence
              ?.map(index => badgeValue[index?.trottype])
              .map((item, idx, arr) => {
                if (idx === arr.length - 1 && arr.length > 1) {
                  return `and ${item}`
                }
                if (idx === arr.length - 3 && arr.length > 2) {
                  return `${item},`
                }
                return item
              })
              .join(' ')} for more accurate results.`}
          </Text>
        </Box>
      )}
    </Box>
